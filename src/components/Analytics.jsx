...
export default function Analytics() {
  ...
  const categoryTrends = useMemo(() => {
    ...
    return { data, topCategories: totals.slice(0, 5) }
          ...
          <div className="mt-3 space-y-2">
            {categoryData.map((item) => (
              <div key={item.name} className="flex items-center justify-between">
                ...
                <span className="text-sm font-medium text-on-surface">{formatLKR(item.value)}</span>
              </div>
            ))}
          </div>
        </section>
            ...
            {categoryChange.map((c) => (
              <div key={c.name} className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className={`flex items-center gap-0.5 text-xs font-medium ${c.change >= 0 ? 'text-error' : 'text-green-400'}`}>
