...
vi.mock('../src/lib/sms', () => ({ requestSmsPermission: vi.fn(async () => false), checkSmsPermission: vi.fn(async () => false), maybeAutoImportSms: vi.fn(async () => []) }))
...
describe('Onboarding', () => {
  ...
  it('shows local account option on choice screen', () => {
    ...
    expect(screen.getByText('Back up with Google')).toBeTruthy()
  })
  ...
  it('navigates to profile screen when choosing local', () => {
    ...
    fireEvent.click(screen.getByText('Continue'))
    fireEvent.click(screen.getByText('Continue'))
    fireEvent.click(screen.getByText('Local account'))
